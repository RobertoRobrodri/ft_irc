...
int		server::server_listening(void)
{
	...
	if ((this->listenin_socket = socket(AF_INET, SOCK_STREAM, 0)) == -1)
		return 0;
	std::cout << "Init socket" << std::endl;
	// No sé que opciones tendremos que habilitar pero vamos a tener que usarlo
	if (setsockopt(this->listenin_socket, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) == -1)
	{
		perror("Bad socket ");
		return 0;
	}
	...
	if (bind(this->listenin_socket, (const sock_addr*)&addr, sizeof(addr)) == -1)
	{
		perror("Error binding ");
		return 0;
	}
	// Tenemos que definir un max_size para la cola
	if (listen(this->listenin_socket, 5) == -1)
	{
